...
def build_model(input_img_shape, model_weights):
...
def stylize(content_img, style_imgs, init_img, frame=None):
    ...
    with tf.device(args.device):
        if prev_net_cache is None or prev_net_cache[0] != content_img.shape:
            ...
            # setup network
            net = build_model(content_img.shape, args.model_weights)
...
def minimize_with_adam(sess, net, optimizer, init_img, loss):
    ...
    sess.run(net['input'].assign(init_img))
    iterations = 0
    with sess.as_default(): # was getting an error without this?
